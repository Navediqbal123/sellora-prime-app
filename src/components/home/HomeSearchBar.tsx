...
const HomeSearchBar: React.FC<HomeSearchBarProps> = ({ value, onChange }) => {
  ...
  return (
    <div className={`relative transition-all duration-300 ${focused ? 'scale-[1.01]' : ''}`}>
      ...
      <div className="relative flex items-center bg-card/80 backdrop-blur-xl border border-border/60 rounded-2xl pl-4 pr-2 h-12 shadow-lg">
        <Search className={`w-5 h-5 mr-2 transition-colors ${focused ? 'text-primary' : 'text-muted-foreground'}`} />
        <input
          type="text"
          value={value}
